<?php
/**
 * nancy functions and definitions.
 */

if ( ! function_exists( 'nancy_setup' ) ) {
	/**
	 ...
	 */
		function nancy_setup() {
			/*
			* Make theme available for translation.
			* Translations can be filed in the /languages/ directory.
			* If you're building a theme based on Crafty Press, use a find and replace
			* to change 'nancy' to the name of your theme in all the template files.
			*/
			load_theme_textdomain( 'nancy', get_template_directory() . '/languages' );
			...
			*/
			...
			register_nav_menus(
				array(
					'primary'        => esc_html__( 'Primary', 'nancy' ),
					'explore'         => esc_html__( 'Explore Menu', 'nancy' ),
					'contact'         => esc_html__( 'Contact Menu', 'nancy' ),
					'socials'         => esc_html__( 'Socials Menu', 'nancy' ),
					'header_action'  => esc_html__( 'Header Action', 'nancy' ),
					'invest'  => esc_html__( 'Invest Industry', 'nancy' ),
					'navigate'  => esc_html__( 'Navigate', 'nancy' ),
					'info'  => esc_html__( 'Info Menu', 'nancy' ),
				)
			);
...
add_action( 'after_setup_theme', 'nancy_setup' );
 ...
 */
function nancy_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'nancy_content_width', 1170 );
}
add_action( 'after_setup_theme', 'nancy_content_width', 0 );
 ...
 * @since 1.0.0
 */
function nancy_sidebar_widgets_init() {
	// Default Sidebar
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'nancy' ),
			'id'            => 'default-sidebar',
			'description'   => esc_html__( 'Add widgets here.', 'nancy' ),
...
add_action( 'widgets_init', 'nancy_sidebar_widgets_init' );

/**
 * Enqueue public scripts and styles.
 */
function nancy_public_scripts() {
    ...
    wp_enqueue_style('google-fonts-oleo', 'https://fonts.googleapis.com/css2?family=Oleo+Script+Swash+Caps:wght@400;700&display=swap', array(), null, 'all');
